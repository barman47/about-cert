<template>
  <div class="mobile-signature-row">
    <div class="top">
      ...
      <div class="file-icon" :class="{pdf: sendMarker.document_name.endsWith('.pdf')}"></div>
      <div class="document-main-details">
        <div class="name">{{sendMarker.document_name}}</div>
        <div>
    ...
    <div class="bottom" v-show="expanded">
      <div class="signatries">
        ...
        <div>
          <div v-for="recipient in sendMarker.recipients " :key="recipient.email">
            <button
              :class="{
        ...
        <div class="status-specific">
          ...
          <div v-else>
            <button
              ...
              @click="$emit('sendSignatureRequest', sendMarker.embedded_signing_url)"
            >Send Now</button>
          </div>
        </div>
      </div>
      <div>
		...
		<div>
			<div>
              <button
                @click="sendMarker.executed != 1 ? {} : $emit('downloadDocument', sendMarker.id)"
...
export default {
  ...
  methods: {
    ...
    expand() {
      this.expanded = true;
      this.$emit("expanded", this.sendMarker.id);
    },
    collapse() {
      this.expanded = false;
    },
